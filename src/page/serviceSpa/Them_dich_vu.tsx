...
const AddService: React.FC = () => {
  ...
  const uploadImagesToCloudinary = async (files: File[]): Promise<string[]> => {
    const cloudinaryUrl = "https://api.cloudinary.com/v1_1/dokp7ig0u/image/upload";
    ...
    for (const file of files) {
      ...
      formData.append("upload_preset", "spamassage");
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
      className="w-full p-10 bg-white rounded-lg shadow-md">
      <ToastContainer />

      <p className="text-2xl font-bold mb-4 text-gray-800">Thêm Dịch Vụ Massage Mới</p>
      <div className="mt-2">
        <label className="block text-gray-700 text-sm font-bold">Thêm danh mục mới
          <i className="ml-1 text-gray-400 font-normal" >(Lưu ý: Không ảnh hưởng đến việc thêm dịch vụ mới.)</i></label>
        <input
          type="text"
          className="w-[30%] p-3 border rounded"
          name="categoryName"
          value={categoryName}
          onChange={(e) => {
        ...
        <button
          type="button"
          className="inline-flex items-center mt-2 px-4 py-2 ml-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 transition-all duration-300"
          onClick={() => {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-10">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-gray-700 text-sm font-bold">Tên dịch vụ</label>
            <input className="w-full p-3 border rounded" placeholder="Tên dịch vụ" {...register("name")} required />
          </div>
          <div>
            <label className="block text-gray-700 text-sm font-bold">Loại dịch vụ</label>
            <input className="w-full p-3 border rounded" placeholder="Loại dịch vụ" {...register("serviceType")} required />
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-gray-700 text-sm font-bold">Giá dịch vụ (VND)</label>
            <input className="w-full p-3 border rounded" type="text" placeholder="Giá (VND)" {...register("price")} required />
          </div>

          <div>
            <label className="block text-gray-700 text-sm font-bold">Thời gian thực hiện (phút)</label>
            <input className="w-full p-3 border rounded" type="text" placeholder="Thời gian (phút)" {...register("duration")} required />
          </div>
        </div>

        <div>
          <label className="block text-gray-700 text-sm font-bold">Mô tả</label>
          <textarea className="w-full p-3 border rounded" placeholder="Mô tả" {...register("description")} required />
        </div>
        <div>
          <label className="block text-gray-700 text-sm font-bold">Danh mục</label>
          <select className="w-full p-3 border rounded" {...register("categoryId")}>
          ...
          {fields.map((step, index) => (
            <div key={step.id} className="flex items-center space-x-2 mb-2">
              <div className="w-full">
                <label className="block text-gray-700 text-sm font-bold m-2">Bước {index + 1}</label>
                <textarea
                  className="w-full h-[80px] p-2 border rounded"
              ...
              <button
                type="button"
                className="bg-red-500 text-white px-3 py-1 rounded rounded-l-full cursor-pointer"
        ...
        <div className="w-full flex items-center justify-center">
          <button
            type="submit"
            className={`w-[40%] ${isLoading ? "bg-gray-400 flex items-center justify-center" : "bg-blue-500 hover:bg-blue-600"
