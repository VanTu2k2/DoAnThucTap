...
// const groupByDate = (invoices: any[]) => {
//   return invoices.reduce((groups: Record<string, any[]>, invoice) => {
...
//       setLoading(true);
//       try {
//         const storedInvoices = JSON.parse(localStorage.getItem("allInvoices") || "[]");
//         const filtered = storedInvoices.filter((invoice: any) =>
//           filterByStatus.includes(invoice.status)
//         );
...
//   const handleConfirmDayBooking = async (invoices: any[]) => {
...
//       const stored = JSON.parse(localStorage.getItem("allInvoices") || "[]");
//       const updated = stored.filter(
//         (inv: any) => !invoices.some((i: any) => i.id === inv.id)
//       );
...
//         const allServices = invoices.flatMap((inv) => inv.serviceIds || []);
...
//                 <Typography><strong>Khách hàng:</strong> {guestNames.join(", ")}</Typography>
        
//                 <Typography><strong>Dịch vụ:</strong></Typography>
//                 <ul className="list-disc ml-6">
//                   {uniqueServices.map((id, idx) => (
//                     <li key={idx}>ID Dịch vụ: {id}</li>
//                   ))}
//                 </ul>
        
//                 <Typography><strong>Chi tiết dịch vụ:</strong></Typography>
//                 <ul className="ml-4 list-disc text-sm space-y-2">
//                   {invoices.map((inv, idx) => (
//                     <li key={idx}>
//                       Thời gian: {new Date(inv.appointmentDateTime).toLocaleTimeString()} - Ghi chú: {inv.notes || "Không có"}
//                     </li>
//                   ))}
//                 </ul>
        
//                 <Typography><strong>Tổng giá:</strong> {totalPrice.toLocaleString()}đ</Typography>
...
//                   >
//                     Xác nhận đặt lịch cho ngày này
//                   </button>
//                 </div>
//               </details>
//             </CardContent>
    ...
    <div className="space-y-6">
      {dateKeys.map((date) => {
        ...
        return (
          ...
          //         <Typography><strong>Tên khách hàng:</strong> {guestNames.join(", ")}</Typography>

          //         <Typography><strong>Gói dịch vụ:</strong></Typography>
          //         <ul className="list-disc ml-6">
          //           {uniqueServices.map((id, idx) => (
          ...
          //         <Typography><strong>Tổng tiền:</strong> {totalPrice.toLocaleString()}đ</Typography>
